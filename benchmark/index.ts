...
function runBenchmark() {
  ...
  const results = [
    ...
    benchmarkCase(
      ...
      (buf) => JSON.parse(gunzipSync(buf).toString()),
    ),
    benchmarkCase(
      'MessagePack',
      () => Buffer.from(msgpackEncode(input)),
      (buf) => msgpackDecode(buf),
    ),

    benchmarkCase(
      'SJT (json)',
      () => Buffer.from(JSON.stringify(encodeSJT(input))),
      (buf) => decodeSJT(JSON.parse(buf.toString())),
    ),
    benchmarkCase(
      'SJT + gzip',
      () => gzipSync(Buffer.from(JSON.stringify(encodeSJT(input)))),
      (buf) => decodeSJT(JSON.parse(gunzipSync(buf).toString())),
    ),
  ];
